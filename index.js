...
const indivdualContributions = GAMES_JSON.reduce( (summ, game) => summ + game.backers, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${indivdualContributions.toLocaleString()}`;
...
function filterUnfundedOnly() {
    ...
    let noFunding = GAMES_JSON.filter ( (game) => {
        return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(noFunding);
}
...
'A total of $' + totalRaised.toLocaleString() + ' has been raised for ' + totalGames + ' games. Currently, ' +
    (unfundedGames.length === 1 ? '1 game remains' : `${unfundedGames.length} games remain`) + 
...
runnerUpGame.innerHTML = `${second.name}`;
